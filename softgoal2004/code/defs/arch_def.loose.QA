...
rule6 says extensibility of function if
	...
	any_one_of helped by sharedData of targetSystem
	any_one_of unhurt by abstractDataType of targetSystem
		with unhurt by c5
	ref "L Chung's book" keywords [chung].      
...
rule8 says modifiability of process if
	...
	any_one_of unbroken by sharedData of targetSystem
		with helped by c2
	ref "L Chung's book" keywords [chung].      

rule9 says modifiability of dataRep if
	unhurt by implicitInvocation of targetSystem
	any_one_of helped by abstractDataType of targetSystem
	any_one_of unbroken by pipeAndFilter of targetSystem
	any_one_of unhurt by sharedData of targetSystem
		with helped by c2
	ref "L Chung's book" keywords [chung].      

rule10 says reusability of system if
	helped by pipeAndFilter of targetSystem
	any_one_of helped by implicitInvocation of targetSystem
	any_one_of unhurt by sharedData of targetSystem
	any_one_of helped by abstractDataType of targetSystem
		with helped by c3
	ref "L Chung's book" keywords [chung].      
...
rule11 says spacePerformance of system if
	unhurt by implicitInvocation of targetSystem
	any_one_of made by sharedData of targetSystem
	any_one_of unbroken by pipeAndFilter of targetSystem
		with made by c4
	ref "L Chung's book" keywords [chung].      
